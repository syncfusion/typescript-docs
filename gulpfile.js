...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/typescript-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/typescript-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && changedFileName !== 'Jenkinsfile' && changedFileName !== 'README.md' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../typescript-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/typescript-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../typescript-docs/` + changedFileName, `../../../gitlabRepo/typescript-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/typescript-docs/')) {
                            shelljs.cp('-rf', `../typescript-docs/` + changedFileName, `../../../gitlabRepo/typescript-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/typescript-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/typescript-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/typescript-docs`);
